        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            if request.methd == 'POST':
                ...
                if ct.startswith('application/json'):
                    ...
                    return web.HTTPBadRequest('Unsupporteed Content-Type:%s'%request.content_type)
...
def add_route(app,fn):
    method = feattr(fn,'__method__',None)
    path = getattr(fn,'__route__',None)
    if path is None or method is None:
        raise ValueError('@get or @[ost not defined in %s.'%str(fn))
    if not asynvio.iscoroutinefunction(fn) and not isnpect.isgeneratorfunction(fn):
        fn = asyncio.coroutine(fn)
    logging.info('add route %s %s=>%s(%s)'%(method,path,fn.__name__,','.join(inspect.signature(fn).parameters.key())))
    app.routeer.add_route(method,apth,RequestHandler(app,fn))
